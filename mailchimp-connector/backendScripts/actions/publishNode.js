    ...
    this.execute = function () {
        ...
        try {
            ...
            if (responseStatus.getStatusCode() === 200) {
                ...
                if (NodeUtil.isNodeType(node, "mlchmp:campaign")) {
                    if (scheduleAt && !scheduled) {
                        try {
                            ...
                            Notification.show("Node published successfully to Mailchimp", Notification.Type.HUMANIZED_MESSAGE)
                                .setDelayMsec(utils.getNotificationDelay());
                        } catch (err) {
                            Notification.show("Failed to publish node with error " + err, Notification.Type.ERROR_MESSAGE)
                                .setDelayMsec(utils.getNotificationDelay());
                        }
                    } else {
                        saveToJCR(node, session);
                ...
                try {
                    Notification.show("Failed to publish node with error " + responseBody.detail, Notification.Type.ERROR_MESSAGE)
                        .setDelayMsec(utils.getNotificationDelay());
                } catch (err) {
                    Notification.show("Failed to publish node with unknown error, API response is " + responseStatus.getReasonPhrase(), Notification.Type.ERROR_MESSAGE)
                        .setDelayMsec(utils.getNotificationDelay());
                }
            }
        } catch(err) {
